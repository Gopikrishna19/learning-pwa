...
const loadMoreRequest = () => {
  ...
  return fetch(endPoint).then(response => response.json()).then(data => __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__template__["a" /* getNodes */])(...data.cars)).then(nodes => nodes.forEach(node => cars.appendChild(node)));
};
