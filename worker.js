...
const cacheFirst = request => caches.match(request).then(response => response || fetchAndCache(request));

const networkFirst = request => fetchAndCache(request).catch(() => caches.match(request));
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.map(key => !(key === CAR_DEALS_CACHE || key === CAR_IMAGES_CACHE) ? caches.delete(key) : null).filter(promise => promise))));
});
